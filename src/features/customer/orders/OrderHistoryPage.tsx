...
const STATUS_MAP: Record<string, { label: string; variant: 'default' | 'secondary' | 'outline' | 'destructive' | 'success' | 'warning' | 'info' }> = {
    '0': { label: 'Đang chuẩn bị', variant: 'warning' },
    '1': { label: 'Đang giao', variant: 'info' },
    '2': { label: 'Đã hoàn thành', variant: 'success' },
                            ...
                            <div className="p-4 bg-white">
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <div>
                                            <span className="font-bold text-gray-900 block truncate max-w-[150px]">Đơn #{order.id.slice(-6).toUpperCase()}</span>
                                    ...
                                    {order.orderLines?.map((line: any, idx: number) => (
                                        <div key={idx} className="flex justify-between text-sm">
                                            <span className="text-gray-600 truncate mr-2">
                                                {line.foodStore?.food?.name || 'Sản phẩm'} <span className="text-gray-400">x{line.quantity}</span>
                                            </span>
                                            <span className="text-gray-900 font-medium whitespace-nowrap">{(line.price * line.quantity).toLocaleString()}đ</span>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="flex items-start text-xs text-gray-400 mb-4 bg-gray-50 p-2 rounded-lg">
                                    ...
                                    <span className="line-clamp-1">{order.store?.address || 'Địa chỉ đã lưu'}</span>
                                </div>
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-gray-100 bg-gray-50/30">
                                    <div>
                                        ...
                                        <span className="text-xl font-extrabold text-orange-600">{(order.totalAmount || 0).toLocaleString()}đ</span>
                                    </div>
